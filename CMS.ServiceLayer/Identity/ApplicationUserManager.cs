...
namespace CMS.ServiceLayer.Identity
{
    ...
    public class ApplicationUserManager : UserManager<User>, IApplicationUserManager
    {
        ...
        public Task<List<User>> GetAllSystemUsersAsync()
        {
            return Users.Where(x => x.IsSystemUser == true).ToListAsync();
        }

        public async Task<IEnumerable<SelectListItem>> GetSystemUsersAsSelectListItem()
        {
            var currentUserId = GetCurrentUserId();
            return await _users.Where(a => a.Id != CurrentUserId && a.IsSystemUser == true)
        ...
        public Task<List<User>> GetAllAnonymousUsersAsync()
        {
            return Users.Where(x => x.IsSystemUser == false).ToListAsync();
        }
